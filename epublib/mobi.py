...
class MobiComicBook(epub.EpubBook):
    ...
    def get_meta_tags(self):
        ...
        for name, content in self.comic_meta:
            tags.append(
                ('<meta name="{0}" content="{1}"/>'.format(name, content),
                 value, end))
        return tags
...
class Page(object):
    ...
    def auto_target(self,
                    whtarget_left, whtarget_top, whtarget_width,
                    whtarget_height,
        ...
        if direction == LEFT:
            chunks = whtarget_width*zoom_factor/100.
            ...
            for i in range(chunk_int):
                ...
                print "H", self.book.height, "MINUS", \
                    - whtarget_height*zoom_factor/2.
                lb_width = 100
                lb_left = 0
                ...
                self.add_zoom_image(
                    zoom_factor,
                    target_left, whtarget_top, target_width,
                    whtarget_height,
                    lb_left, 50. - whtarget_height*zoom_factor/2.,
                    lb_width, whtarget_height*zoom_factor,
                    -zoom_left, calculated_zoom_something,
...
zl:{8} zt:{9}""".format(
        ...
        mag = Mag(txt, tid+self.PARENT_SUFFIX, tid,
                  int(target_left), int(target_top),
...
def test():
    ...
    for col in range(4):
        # target_left, target_top, target_width, target_height,
        page.auto_target(col*25, 500./6, 25, 100./6,
                         zoom_factor=2)
